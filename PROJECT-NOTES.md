...
- **Refer a Friend** — lets *anyone* viewing a card (not just its owner) share a link, via a Text/Email/Copy choice. Branches on the person's `isRealtor` flag: a realtor's card sends the realtor pricing schedule (`pricing-schedule-realtors.html`) with listings-focused wording, everyone else gets the general intake form with "Request a Black Hat Card" wording. Both point at fixed URLs (never derived from the current page's domain), so they're correct even viewed via an old `.vercel.app` tag.
...
- **Brand system** — `brand-generator.html` creates a `brands/{slug}.json` file (business name, accent color + soft variant, and an optional background color). Any person's card *or* any property listing can reference that slug via a **Brand** field in `generator.html` / `property-generator.html`, and the live card fetches and applies those colors at runtime (`applyBrandTheme` in `app.js` and `property.js`). Both generators' own **Preview** button also fetches and applies the real brand colors before showing the card — not a simulation, the same fetch-and-override logic the live card itself uses, scoped to just the preview element. One brand file, edited once in `brand-generator.html`, updates every person and listing referencing it — no per-card edits needed to rebrand. A brand file works identically whether multiple people reference it (a real business/team) or just one (personal variety with no business behind it) — nothing in the code distinguishes the two cases.
- **Background color and automatic text contrast** — a brand file's `background` field is optional; when set, `computeThemeFromBackground` (duplicated identically in `app.js`, `property.js`, and `brand-generator.html`'s own preview, deliberately kept in sync across all three) derives a readable text color plus matching panel/border shades from that background's relative luminance, rather than requiring anyone to pick a text color by hand. This is why `html, body`'s background in `style.css` was simplified from a radial gradient with one hardcoded stop to a flat `var(--ink)` — the old gradient couldn't fully respect an arbitrary override. Leaving `background` unset falls back to the original dark theme exactly as it always worked, confirmed against Angela's own brand file, which predates this feature and has no `background` key at all.
- **`brands/` naming convention: `biz-` vs `solo-` prefix** — not enforced by any code, purely an operating discipline adopted once personal (single-person) brand files became a real use case alongside team brands. `biz-{slug}.json` signals an actual business multiple people may reference (e.g. `biz-the-real-estate-co.json`, Angela's team); `solo-{slug}.json` signals one person's own personal variety with no business behind it. Adopted retroactively for Angela's own brand file (originally `the-real-estate-co.json`, renamed to `biz-the-real-estate-co.json` — all 6 references updated: her own `data.json` plus all 5 listings').
...
- **Listings** (person cards only) — lets a realtor's *personal* card link out to their active property listings. Deliberately not available on the friend-facing intake form, since a brand-new person filling out their own card wouldn't have live listing URLs to reference yet — this is a field you add after the fact, in the generator. Capped at 5 for most people, 10 for realtor-tier cards (`isRealtor: true`) — checked in three places that all need to agree: `app.js`'s render, and `generator.html`'s UI-disable logic and `collectData()`. The generator's Add Listing button also updates its own limit live if the realtor checkbox is toggled after some listings are already entered, rather than requiring a page reload to pick up the new cap.
- **Realtor tier (`isRealtor` flag)** — set via a checkbox in `generator.html`, just above Property Listings. Changes what Refer a Friend sends (see above) and raises the listings cap to 10 (see above). Doesn't apply retroactively; an existing person's card (Angela's, specifically) needs to be Loaded, the box checked, and re-saved before either behavior takes effect.
...
- **Realtor teams nest three deep: agency → person → that person's listings** (e.g. `the-real-estate-co/angela-hanks/4098-charlie-harris-rd/`), first adopted for Angela Hanks' team. Deliberately *not* nested under the team lead's own name — a teammate isn't "inside" the broker-owner's folder, they're a peer who happens to share the same agency, so the agency itself (matching its brand slug) is the top-level container, with every agent, owner included, sitting as a sibling underneath it. Established before any physical tags existed for that team specifically so the restructure was free; doing this after tags exist means fixing URLs inside already-written chips, not just moving folders.
