...
- **Automatic CC on a sensitive-field change** — both `my-contact-info-form.html` and `property-intake-form.html` capture the phone/email (or `agentPhone`/`agentEmail`) originally on file the moment prefill data loads, before any edits happen. On submit, if either field no longer matches what was originally captured, the outgoing `mailto:` link automatically CCs the *original* email address alongside sending to `RECIPIENT_EMAIL`, and the message body gets a short appended note explaining why that address is CC'd and asking the actual cardholder to reply right away if they didn't request the change — without it, an unexplained CC risks reading as confusing noise rather than the actionable flag it's meant to be, for both a genuine sender (who'd otherwise wonder why their old email is suddenly included) and the CC'd party themselves. The note only gets appended when a CC actually fires — an ordinary update with no sensitive-field change stays exactly as it always looked. Costs nothing if the request is genuine — the real person just sees a copy of their own message with a plain explanation — but means an actual cardholder still sees it happen (and knows to act on it) if someone else is the one asking for the change. Only triggers for an existing person/listing's edit (`prefillData` present, something already on file to compare against) — a brand-new request has nothing to compare against, so no CC ever gets added there. Doesn't touch phone-number-based notification at all, since SMS has no CC equivalent — email is the only channel this works through, which is why it fires for *either* a phone or an email change, always notifying the old email specifically. Deliberately CC, not BCC — CC leaves a visible record in the header of every message that actually triggered this, where BCC would hide it from the recipient reading the request too, not just from anyone else. See `MANUAL.md` §20 for the full reasoning and the manual habits (confirming via old contact info, a private verification phrase) this supplements rather than replaces.
...
Moved to its own file — `FUTURE-CONSIDERATIONS.md` — once this section got long
enough to make the rest of this document harder to navigate. Check there for
known gaps, deferred architectural decisions, and things being watched but not
yet acted on.
